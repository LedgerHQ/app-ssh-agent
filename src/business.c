...
unsigned char bip32_print_path(uint32_t *bip32Path,
                               unsigned char bip32PathLength, char *out,
...
void ins_get_public_key(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
...
#ifdef TARGET_BLUE
    os_perso_derive_node_bip32(CX_CURVE_Ed25519, operationContext.bip32Path,
                               operationContext.pathLength, privateKeyData,
                               NULL);
#else
    os_perso_derive_node_bip32_seed_key(
        HDW_ED25519_SLIP10, CX_CURVE_Ed25519, operationContext.bip32Path,
        operationContext.pathLength, privateKeyData, NULL,
        (unsigned char *)"ed25519 seed", 12);
#endif
  } else {
    os_perso_derive_node_bip32(curve, operationContext.bip32Path,
                               operationContext.pathLength, privateKeyData,
                               NULL);
  }
#else
  ...
  if (curve == CX_CURVE_Ed25519) {
    ...
    cx_ecfp_generate_pair(curve, &operationContext.publicKey, &privateKey, 1);
  }
#else
  cx_ecfp_generate_pair(curve, &operationContext.publicKey, &privateKey, 1);
#endif
  ...
  bip32_print_path(operationContext.bip32Path, operationContext.pathLength,
...
static void copy_message(uint8_t *dataBuffer, uint32_t available) {
  ...
  memmove(operationContext.message + operationContext.messageLength, dataBuffer,
          available);
  operationContext.messageLength += available;
}

static void read_length(uint8_t *dataBuffer, uint32_t dataLength,
                        uint8_t available) {
  (void)dataLength;
...
static void read_user_name(uint8_t *dataBuffer, uint32_t dataLength) {
  ...
  memmove(operationContext.userName + operationContext.userOffset, dataBuffer,
          userAvailable);
  operationContext.userOffset += userAvailable;
}

static void read_element(uint8_t *dataBuffer, uint32_t dataLength,
                         uint32_t available) {
...
static void read_blob(uint8_t **pDataBuffer, uint32_t *pDataLength,
                      bool have_length) {
  uint8_t *dataBuffer = *pDataBuffer;
  uint32_t dataLength = *pDataLength;
  uint32_t available;
...
void ins_sign_ssh_blob(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  operationContext.userName[operationContext.userOffset] = '\0';
  bip32_print_path(operationContext.bip32Path, operationContext.pathLength,
                   keyPath, sizeof(keyPath));
...
void ins_sign_generic_hash(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  cx_hash(&operationContext.hash.header, 0, dataBuffer, dataLength, NULL, 0);
...
void ins_get_ecdh_secret(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  operationContext.curve = get_curve(p2);
  cx_ecfp_init_public_key(operationContext.curve, dataBuffer, 65,
