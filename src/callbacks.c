...
unsigned int io_seproxyhal_touch_sign_ok(const bagl_element_t *e) {
  ...
  if (operationContext.curve == CX_CURVE_Ed25519) {
    if (!operationContext.fullMessageHash) {
#if CX_APILEVEL >= 8
      tx = cx_eddsa_sign(&privateKey, CX_LAST, CX_SHA512, hash, sizeof(hash),
                         NULL, 0, G_io_apdu_buffer, sizeof(G_io_apdu_buffer),
                         NULL);
#else
      ...
      tx = cx_eddsa_sign(&privateKey, CX_LAST, CX_SHA512,
                         ...
                         operationContext.messageLength, NULL, 0,
    ...
    tx = cx_ecdsa_sign(&privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256, hash,
                       sizeof(hash), G_io_apdu_buffer, sizeof(G_io_apdu_buffer),
                       &info);
  ...
  if (operationContext.getPublicKey) {
    ...
    if (operationContext.curve == CX_CURVE_Ed25519) {
      ...
      cx_ecfp_generate_pair(operationContext.curve, &operationContext.publicKey,
                            &privateKey, 1);
    }
#else
    cx_ecfp_generate_pair(operationContext.curve, &operationContext.publicKey,
...
unsigned int io_seproxyhal_touch_ecdh_ok(const bagl_element_t *e) {
  ...
  tx = cx_ecdh(&privateKey, CX_ECDH_POINT, operationContext.publicKey.W,
               operationContext.publicKey.W_len, G_io_apdu_buffer,
