...
void ui_idle(void) {
  nbgl_useCaseHome(APPNAME, &C_icon_app_ssh_64px, "Application is ready.",
                   false, app_settings, app_quit);
}
...
void ui_get_public_key(void) {
  nbgl_useCaseChoice(&C_warning64px, "Provide public key?", NULL, "Allow",
                     "Reject", ui_get_public_key_choice);
}
...
void ui_sign_ssh_blob(void) {
  nbgl_useCaseChoice(&C_warning64px, "Connect SSH user",
                     (char *)operationContext.userName, "Allow", "Reject",
...
void ui_get_ecdh_secret(void) {
  nbgl_useCaseChoice(&C_warning64px, "PGP Agent", "Allow ECDH?", "Allow",
                     "Reject", ui_get_ecdh_secret_choice);
}
...
void ui_sign_direct_hash(void) { ui_sign_generic_hash(); }
